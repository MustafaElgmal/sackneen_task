...
export const getFavoriteMovieById = (id: number, user: userType) => {
  const errors: { message: string }[] = [];
  const favoriteMovie = user.favoriteMovies.find((movie) => movie.id === id);
  if (!favoriteMovie) {
    errors.push({ message: "Favorite Movie is not found!" });
  }
